...
export default (props: Props) => {
    ...
    return <div className="program_header_container">
        <header>
            <img src={props.program.thumbnail} />
        </header>
        <div className="trip_lodge pl-2 mt-3">
            <p>{props.program.lodge}</p>
            <p className="share_trip"><ShareIcon></ShareIcon> Share this trip</p>
        </div>
        <div className="heading-3 pl-2">{props.program.title}</div>
        <div className="subtitle pl-2">{props.program.subtitle}</div>
        <div className="dates pl-2"><p>From <strong>{moment(props.program.from).format('MMM Do')}</strong></p> <p>to <strong>{moment(props.program.to).format('MMM Do')}</strong></p></div>
        <style>
          {
            `
