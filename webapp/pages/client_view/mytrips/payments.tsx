        ...
        <div className="payment_info p-2 m-5 rounded-lg border-2">
          ...
          <article className="payment_items mt-5">
            { payment.charges.map( (charge:any) => {
              return <div key={charge.text} className="payment_charge mt-1"><p>{charge.text}</p><p>${charge.amount}</p></div>
            })}
          </article>
          <article className="payment_total mt-5 pt-3 border-dashed border-t-2">
            ...
            <div className="mt-4"><CreditCard color="black"></CreditCard> Your Payment</div>
          ...
          <article className="payment_balance mt-5 pt-3 border-dashed border-t-2">
            ...
            <div className="mt-5 text-center">
              <button className="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded-full shadow-md"><CreditCard color="white"></CreditCard> Pay Balance</button>
            </div>
          </article>
          <article className="payment_download pl-5 mt-5 pt-3 border-solid border-t-2">
            <Download></Download><p className="text-blue-500">DOWNLOAD INVOICE</p>
          </article>
          <article className="payment_reservation pl-5 mt-5 pt-3 border-solid border-t-2">
            <p>Reservation Nr. #{payment.reservation}</p>
            <p>Invoice date: {moment(payment.invoice_date).format('Do MMM')}</p>
          </article>
        </div>
        <style>
            {
...
const CreditCard = (color: string) => (<svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M17.5 3.5H3.5C2.52875 3.5 1.75875 4.27875 1.75875 5.25L1.75 15.75C1.75 16.7212 2.52875 17.5 3.5 17.5H17.5C18.4713 17.5 19.25 16.7212 19.25 15.75V5.25C19.25 4.27875 18.4713 3.5 17.5 3.5ZM17.5 15.75H3.5V10.5H17.5V15.75ZM3.5 7H17.5V5.25H3.5V7Z" fill={color.color} fill-opacity="0.54"/>
</svg>
)

const Download = () => (<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.59 7.09L10 9.67V0H8V9.67L5.41 7.09L4 8.5L9 13.5L14 8.5L12.59 7.09ZM16 16V9H18V16C18 17.1 17.1 18 16 18H2C0.9 18 0 17.1 0 16V9H2V16H16Z" fill="#4299E1"/>
</svg>
)